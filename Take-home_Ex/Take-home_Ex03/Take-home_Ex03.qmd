...
```{r}
mpsz_exc <- mpsz |> filter(SUBZONE_N == "JURONG ISLAND AND BUKOM" | SUBZONE_N == "SUDONG" | SUBZONE_N == "SEMAKAU" | SUBZONE_N == "SOUTHERN GROUP" | SUBZONE_N == "NORTH-EASTERN ISLANDS")
mpsz_exc_union <- mpsz_exc |> st_union()
mpsz_union <- mpsz |> st_union()
mpsz_main_union <- st_difference(mpsz_union, mpsz_exc_union)
mpsz_main <- mpsz |> st_intersection(mpsz_main_union) 
mpsz2 <- select(mpsz, SUBZONE_N, geometry)
mpsz_main2 <- mpsz_main |> st_drop_geometry()
mpsz_main3 <- mpsz2 |> inner_join(mpsz_main2)
```
...
```{r}
mpsz_main3 <- st_set_crs(mpsz_main3, 3414)
```
...
median_price <- listing_clean |> 
  group_by(neighbourhood_cleansed) |> 
  summarise_at(vars(price), list(median_price = median))

max_price <- listing_clean |> 
  group_by(neighbourhood_cleansed) |> 
  summarise_at(vars(price), list(max_price = max))

min_price <- listing_clean |> 
  group_by(neighbourhood_cleansed) |> 
  summarise_at(vars(price), list(min_price = min))
```
...
```{r}
airbnb <- mpsz_main3 |> left_join(aggregate_price, by = c("PLN_AREA_N" = "neighbourhood_cleansed"))
...
mean <- tm_shape(airbnb) + 
  tm_fill("mean_price", n = 5, breaks = c(0,50, 115.6, 145.5, 235.5,max(airbnb$mean_p)), palette = "YlOrRd") +
  tm_borders() + 
  tm_layout(main.title = "Mean prices of airbnb listing")
median <- tm_shape(airbnb) + 
  tm_fill("median_price", n = 5, style = "quantile") +
  tm_borders() + 
  tm_layout(main.title = "Median prices of airbnb listing")
max <- tm_shape(airbnb) + 
  tm_fill("max_price", n = 5, style = "quantile") +
  tm_borders() + 
  tm_layout(main.title = "Max prices of airbnb listing")
min <- tm_shape(airbnb) + 
  tm_fill("min_price", n = 5, style = "quantile") +
  tm_borders() + 
...
```{r}
localMI_mean <- localmoran(airbnb$mean_price, rswm_q)
localMI_median <- localmoran(airbnb$median_price, rswm_q)
localMI_max <- localmoran(airbnb$max_price, rswm_q)
localMI_min <- localmoran(airbnb$min_price, rswm_q)
```
...
```{r}
airbnb.localMI_mean <- cbind(airbnb, localMI_mean) |>
...
map_i_mean <- tm_shape(airbnb.localMI_mean) +
  ...
  tm_borders(alpha = 0.5) +
   tm_layout(main.title = "Local Moran Statistic of mean Airbnb prices" , title.size = 1)
map_i_mean
...
map_pval_mean <- tm_shape(airbnb.localMI_mean) +
  ...
  tm_borders(alpha = 0.5) + 
   tm_layout(main.title = "Local Moran p-value of mean Airbnb prices", title.size = 1)
...
signif <- 0.05  
quadrant[airbnb.localMI_mean$Pr.Ii>signif] <- 0
...
quadrant2[airbnb.localMI_median$Pr.Ii>signif] <- 0
...
quadrant3[airbnb.localMI_max$Pr.Ii>signif] <- 0
...
quadrant4[airbnb.localMI_min$Pr.Ii>signif] <- 0
```
